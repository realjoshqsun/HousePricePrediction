...
S_train_gbdt = np.zeros((dataset.shape[0]))

for train_index, test_index in kf.split(x_data):
    train_x, train_y = x_data[train_index], y_data[train_index]
    test_x, test_y = x_data[test_index], y_data[test_index]

    gbdt = GradientBoostingRegressor(max_depth=15,min_samples_split=2)
    gbdt.fit(train_x, train_y)
    S_train_gbdt[test_index] = gbdt.predict(test_x)

    print("训练集均方误差为:", mean_squared_error(gbdt.predict(train_x,), train_y))
    print("验证集均方误差为:", mean_squared_error(gbdt.predict(test_x), test_y))

    preds = gbdt.predict(test)
...
pediction.to_csv('gbdt_kfold.csv',encoding = 'utf-8',columns=columns,index=None)

S_train_gbdt = pd.DataFrame(S_train_gbdt)
S_train_gbdt.to_csv('gbdt_train.csv',encoding = 'utf-8')
